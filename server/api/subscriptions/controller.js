...
exports.getSubscription = async (subscription_id) => {
  let query =
    subscription_id === "" ? { is_default: true } : { id: subscription_id };
  ...
  return await Subscription.findAll({
    ...
    .then((subscription) => {
      ...
      else return { status: 400, error: "No Subscriptions" };
    })
    .catch((err) => {
  ...
  Subscription.findByPk(data.id)
    .then((subscription) => {
      ...
      if (!subscription.length > 1) {
        return res
          .status(401)
          .send({ error: "Multiple subscriptions found for the id" });
      }
...
exports.deleteSubscription = (req, res) => {
  ...
  Subscription.destroy({
    ...
    .then((response) => {
      if (response === 1) return res.json("Subscription deleted successfully.");
      else
